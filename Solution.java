...
class Decoding {
    //    use a queue ds to iterate through the characters
    //    an inner loop to iterate through the tree
    void decode(String s, Node root) {
        ...
        while (!chars.isEmpty()) {
            ...
            while (true) {
                char currentChar = chars.remove();

                if (currentChar == '0' && temporaryRoot.left != null) {
                    temporaryRoot = temporaryRoot.left;
                }
                else if (currentChar == '1' && temporaryRoot.right != null) {
                    temporaryRoot = temporaryRoot.right;
                }

                if (temporaryRoot.left == null && temporaryRoot.right == null) {
                    decodedString.append(temporaryRoot.data);
                    break;
                }
            }
        }
